...
class DiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, **block_kwargs):
        ...
        self.attn_f = Attention(hidden_size, num_heads=num_heads, qkv_bias=True, **block_kwargs)
        self.attn_mid = Attention(hidden_size, num_heads=num_heads, qkv_bias=True, **block_kwargs)
        self.norm2 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        mlp_hidden_dim = int(hidden_size * mlp_ratio)

        self.cross_attn_s= nn.MultiheadAttention(hidden_size, num_heads)
        self.cross_attn_f= nn.MultiheadAttention(hidden_size, num_heads)
    ...
    def forward(self, x, c, f):
        ...
        f = f + self.attn_f(self.norm1(f))
        q = self.norm1(x).permute(1, 0, 2)
        k = self.norm1(f).permute(1, 0, 2)
        v = self.norm1(f).permute(1, 0, 2)
        
        cross, _ = self.cross_attn_f(q,k,v)
        x = x + cross.permute(1, 0, 2)

        x = x + self.attn_mid(self.norm1(x))

        
        q = self.norm1(x).permute(1, 0, 2)
        k = self.norm1(c).permute(1, 0, 2)
        v = self.norm1(c).permute(1, 0, 2)
        # cross attention
        cross, _ = self.cross_attn_s(q,k,v)
        x = x + cross.permute(1, 0, 2)
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.yImg_embedder = nn.Linear(1280, hidden_size)
    ...
    def forward(self, x, t, y, f):
        ...
        features_f, coords_f = f, f
        f, _ = self.voxelization(features_f, coords_f)

        f = self.x_embedder(f) 
        f = f + self.pos_embed # (N, 512, 384)

        x = self.x_embedder(x) 
        x = x + self.pos_embed 

        
        y = self.yImg_embedder(y) # (N, 257, 384)

        # y = self.y_embedder(y, self.training)    
        # yとfをconcat
        # y = torch.cat([y, f], dim=1)# (N, 257+512, 384)

        t_c = self.t_embedder(t)  
        t_c = t_c.view(t_c.shape[0], 1, t_c.shape[1]).repeat(1, y.shape[1], 1) # (N, 257, 384)

        c = t_c + y  

        t_f = self.t_embedder(t)  
        t_f = t_f.view(t_f.shape[0], 1, t_f.shape[1]).repeat(1, f.shape[1], 1) # (N, 257, 384)      
        f = t_f + f        
