...
from data_module.data_module import PointCloudPartSegWhithSketch
...
import wandb
wandb.init(project="cross-self-fix-sketch")
...
class GaussianDiffusion:
    ...
    def p_losses(self, denoise_fn, data_start, t, noise=None, y=None, f = None):
        """
        ...
        if self.loss_type == 'mse':
            ...
            eps_recon = denoise_fn(data_t, t, y, f)
...
class Model(nn.Module):
    ...
    def get_loss_iter(self, data, noises=None, y=None, f= None):
        ...
        losses = self.diffusion.p_losses(
            denoise_fn=self._denoise, data_start=data, t=t, noise=noises, y=y, f=f)
...
def train(gpu, opt, output_dir, noises_init):
    ...
    for epoch in range(start_epoch, opt.niter):
        ...
        with tqdm(dataloader, desc=f"Epoch {epoch+1}") as tqdmbar:
            for i, data in enumerate(tqdmbar):
                ...
                loss = model.get_loss_iter(x, noises_batch, y, f).mean()
...
def parse_args():
    ...
    # Data params
    parser.add_argument('--dataroot', default='../Dataset/PointsPartSegWithSketch/')
